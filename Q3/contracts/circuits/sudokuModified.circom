...
template sudoku() {
    ...
    component puzzleRangeChecks[9][9];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeChecks[i][j] = RangeProof(32);
        }
    }

    component solutionRangeChecks[9][9];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            solutionRangeChecks[i][j] = RangeProof(32);
        }
    }

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeChecks[i][j].in <== puzzle[i][j];
            puzzleRangeChecks[i][j].range[0] <== 0;
            puzzleRangeChecks[i][j].range[1] <== 9;
            puzzleRangeChecks[i][j].out === 1;
            solutionRangeChecks[i][j].in <== solution[i][j];
            solutionRangeChecks[i][j].range[0] <== 0;
            solutionRangeChecks[i][j].range[1] <== 9;
            solutionRangeChecks[i][j].out === 1;
